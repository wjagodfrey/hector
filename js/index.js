...
(function() {
  var entities, events, fireEvent, frame, frameCount, frameIndex, gameCanvas, gameCq, group, groupName, loaded, loadedFrameCount, mouse, onEvent, root, sprite, spriteName, sprites, _fn, _ref, _ref1;
  ...
  onEvent = function(eventName, eventHandler) {
    ...
    return function() {
      ...
      return console.log(events[eventName]);
    };
  };

  fireEvent = function(eventName) {
    var eventHandler, _i, _len, _ref, _results;
    if (events[eventName].length) {
      _ref = events[eventName];
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        eventHandler = _ref[_i];
        _results.push(eventHandler());
      }
      return _results;
    }
  };
  ...
  for (spriteName in sprites) {
    ...
    for (groupName in _ref) {
      ...
      _ref1 = group.frames;
      _fn = function(group, frameIndex, frame) {
        var img;
        frameCount++;
  ...
  onEvent('assetsLoaded', function() {
    ...
    return entities.push(this.face = face = {
      ...
      draw: function(ctx) {
        var earPax, eyeOffset, eyeballPax, eyebrowOffset, faceMargin, middleSkinPax, mouthOffset, noseOffset, nosePax, pax, pupilPax, resizeFactor, topSkinPax;
        pax = {
          x: (mouse.x - gameCanvas.width / 2) / gameCanvas.width / 2,
          y: (mouse.y - gameCanvas.height / 2) / gameCanvas.height / 2
        };
        faceMargin = 7;
        resizeFactor = Math.min(gameCanvas.height / this.a.skin[0].height - faceMargin, gameCanvas.width / this.a.ears[0].width - faceMargin);
        if (resizeFactor < 2) {
          resizeFactor = 2;
        }
